...
<li class="select-none">
	{#if isFolder(node)}
		<button
			type="button"
			class="flex w-full items-start gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-200 dark:hover:bg-slate-900"
			style={`padding-left: ${indent}`}
			aria-expanded={expanded}
			onclick={() => onToggle(node.id)}
		>
			<span
				class={`flex h-5 w-4 flex-shrink-0 items-center justify-center text-slate-400 transition-transform ${
			...
			<span class="leading-5">{node.name}</span>
		</button>
	...
	{:else}
		<div
			class="flex w-full items-start gap-2 rounded-lg px-3 py-2 text-sm text-slate-500 dark:text-slate-400"
			style={`padding-left: ${indent}`}
		>
			<span class="flex h-5 w-5 flex-shrink-0 items-center justify-center" aria-hidden="true">
